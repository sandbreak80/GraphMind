#!/usr/bin/env python3
"""Test with 25 prompts using 4 different Ollama models based on complexity."""
import requests
import time
import statistics
import json
...
def test_25_4_models():
    ...
    print("\n🚀 25 PROMPTS - 4 DIFFERENT MODELS TEST")
    print("=" * 80)
    print(f"⏰ Started at: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("📝 Testing: 25 prompts across 4 modes with 4 different models")
    print("🧵 Mode: Single-threaded (GPU-optimized)")
    print("⏱️  Patience: Will wait for ALL responses to complete")
    print("🤖 Models: Small, Medium, Large, Thinking")
    print("=" * 80)
    
    # Model mapping based on complexity
    model_mapping = {
        "simple": "llama3.2:3b",      # Small model for simple queries
        "medium": "llama3.1:latest",  # Medium model for medium queries  
        "complex": "qwen2.5-coder:14b", # Large model for complex queries
        "research": "deepseek-r1:latest" # Thinking model for research queries
    }
    
    # 25 test prompts with varying complexity
    timestamp = int(time.time())
    test_prompts = [
        # Simple queries (6) - Small model
        ...
        # Research queries (4) - Thinking model
    ...
    model_results = {"llama3.2:3b": [], "llama3.1:latest": [], "qwen2.5-coder:14b": [], "deepseek-r1:latest": []}
    
    print(f"\n🧪 Testing {len(test_prompts)} prompts across {len(test_modes)} modes with 4 models")
    print(f"📊 Total tests: {len(test_prompts) * len(test_modes)}")
    print(f"🤖 Model mapping:")
    print(f"   Simple queries: {model_mapping['simple']} (Small)")
    print(f"   Medium queries: {model_mapping['medium']} (Medium)")
    print(f"   Complex queries: {model_mapping['complex']} (Large)")
    print(f"   Research queries: {model_mapping['research']} (Thinking)")
    print(f"⏱️  Estimated time: {len(test_prompts) * len(test_modes) * 15} seconds (15s per query)")
    print("-" * 80)
    ...
    for prompt_idx, prompt_data in enumerate(test_prompts, 1):
        ...
        for mode_idx, mode in enumerate(test_modes, 1):
            ...
            try:
                ...
                model_results[used_model].append(result)
    ...
    for model, model_data in model_results.items():
        if model_data:
            successful_model = [r for r in model_data if r['success']]
            if successful_model:
                model_times = [r['response_time'] for r in successful_model]
                cached_count = sum(1 for r in successful_model if r['is_cached'])
                print(f"   {model}:")
                print(f"      Success rate: {len(successful_model)}/{len(model_data)} ({len(successful_model)/len(model_data)*100:.1f}%)")
                print(f"      Avg response time: {statistics.mean(model_times):.2f}s")
                print(f"      Cached responses: {cached_count}/{len(successful_model)} ({cached_count/len(successful_model)*100:.1f}%)")
    ...
    # Overall grade
    if success_rate >= 95 and avg_response_time < 10.0 and cache_hit_rate >= 30:
        print("\n🎉 OVERALL GRADE: A+ (EXCELLENT)")
        print("   System is performing exceptionally well!")
    elif success_rate >= 90 and avg_response_time < 30.0 and cache_hit_rate >= 15:
        print("\n✅ OVERALL GRADE: A (VERY GOOD)")
        print("   System is performing very well!")
    elif success_rate >= 80 and avg_response_time < 60.0:
